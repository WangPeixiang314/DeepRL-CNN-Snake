...
def is_collision_py(snake, width, height, pos=None):
    """检测碰撞"""
    if pos is None:
        pos = snake[0]
...
def random_position_py(width, height, BLOCK_SIZE):
    """生成随机位置"""
    return (
...
def place_food_py(snake, width, height, BLOCK_SIZE):
    """放置食物，避开蛇身"""
    while True:
        food = random_position_py(width, height, BLOCK_SIZE)
        if food not in snake:
            return food


def update_direction_py(current_dir, action):
    """根据动作更新方向"""
    # 获取当前方向的索引
...
def step_logic_py(
    ...
    """
    # 1. 更新方向
    new_direction_vec = update_direction_py(direction_vec, action)
    ...
    if is_collision_py(new_snake, width, height, new_head) or steps_since_food >= MAX_STEPS_WITHOUT_FOOD:
    ...
    # 4. 检查吃到食物
        ...
        # 计算距离奖励
        distance = distance_py(new_head, food)
...
def propagate_py(tree, idx, change):
    parent = (idx - 1) // 2
    while parent >= 0:
        tree[parent] += change
        if parent == 0:
...
def retrieve_py(tree, capacity, s):
    idx = 0
...
def safe_action_py(q_values, danger_signals, collision_penalty, state):
    """
    防自杀机制的核心逻辑 (numba加速)
...
def get_head_pos_py(head, width, height):
    """获取蛇头坐标 (归一化)"""
    return np.array([
        head[0] / width,
        head[1] / height
...
def get_food_pos_py(food, width, height):
    """获取食物坐标 (归一化)"""
    return np.array([
        food[0] / width,
        food[1] / height
...
def get_relative_distance_py(head, food, width, height):
    """获取与食物的相对距离 (归一化)"""
    return np.array([
        (food[0] - head[0]) / width,
...
def get_manhattan_distance_py(head, food, width, height):
    """获取与食物的曼哈顿距离 (归一化)"""
...
def get_eight_direction_dangers_py(head, snake, width, height, BLOCK_SIZE):
    """八方向的三格危险检测"""
    eight_directions = [
        (1, 0),   # 右
        (1, 1),   # 右下
    ...
    for dir_idx, (dx, dy) in enumerate(eight_directions):
        for step in range(1, 4):  # 检测1-3格距离
            ...
            direction_dangers[danger_idx] = is_collision_py(
                snake, width, height, (check_x, check_y)
            )
    return direction_dangers


def get_boundary_distances_py(head, width, height):
    """蛇头到四边界的距离 (归一化)"""
    return np.array([
        head[0] / width,                    # 左边界
...
def get_snake_length_py(snake, grid_area):
    """当前蛇的长度 (归一化)"""
    return np.array([len(snake) / grid_area], dtype=np.float32)


def get_free_space_ratio_py(snake, grid_area):
    """剩余空格比例 (归一化)"""
    return np.array([(grid_area - len(snake)) / grid_area], dtype=np.float32)


def get_local_grid_view_py(head, snake, food, width, height, BLOCK_SIZE):
    """以蛇头为中心的6*6局部网格状态"""
...
def get_all_grid_view_py(snake, food, width, height, BLOCK_SIZE):
    """获取整个地图的直接网格信息并展平为向量（Numba加速版）"""
...
def get_action_history_onehot_py(action_history):
